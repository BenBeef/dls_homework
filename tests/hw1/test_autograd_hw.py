...
def gradient_check(f, *args, tol=1e-6, backward=False, **kwargs):
    ...
    if not backward:
        ...
        out.backward()
        print("types:", [type(a) for a in args])
        computed_grads = [a.grad.numpy() for a in args]

    for i in range(len(args)):
        if f.__name__.find("mat") > -1:
            if len(computed_grads[i].shape) == 3:
                print(f"computed_grads[{i}]:\n", computed_grads[i])
                print(f"numerical_grads[{i}]:\n", numerical_grads[i])
                print(f"np.linalg.norm(computed_grads[{i}] - numerical_grads[{i}]):", np.linalg.norm(computed_grads[i] - numerical_grads[i]))

    error = [np.linalg.norm(computed_grads[i] - numerical_grads[i]) for i in range(len(args))]
    print(f, error)
    error = sum(
        error
    )
...
def test_matmul_batched_backward():
    ...
    gradient_check(
        ndl.matmul,
        ndl.Tensor(np.random.randn(2, 4, 2)),
        ndl.Tensor(np.random.randn(2, 3)),
    )
    gradient_check(
        ndl.matmul,
        ndl.Tensor(np.random.randn(5, 4)),
