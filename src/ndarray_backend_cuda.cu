...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  if (gid < size) {
      ...
      for (int i=shape.size-1; i > -1; i --) {
          size_t s = shape.data[i];
          cnt += (r % s) * strides.data[i];
          r =  r / s;
      }
      out[gid] = a[offset + cnt];
  }
  /// END SOLUTION
}
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
    ...
    /**
     * Args:
     *   a: compact represntation of the array, given as input
     *   out: non-compact version of the array to be written
     *   shape: shapes of each dimension for a and out
...
__global__ void ReduceMaxKernel(const scalar_t* a, scalar_t* out, size_t size, size_t reduce_size) {
    size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
    if (gid >= size || reduce_size <= 0) return;
    scalar_t max = a[gid * reduce_size + 0];
    for (int i=1; i < reduce_size; i++) {
        if (max < a[gid * reduce_size + i]) {
