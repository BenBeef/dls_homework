...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for i in reverse_topo_order:
        arr, vi = node_to_output_grads_list[i], node_to_output_grads_list[i][0]
        for v in arr[1:]:
            vi = needle.ops.add(vi, v)
        i.grad = vi
        if i.op is None:
            continue
        grads = i.op.gradient_as_tuple(vi, i)
        for j, _input in enumerate(i.inputs):
            node_to_output_grads_list.setdefault(_input, [])
            node_to_output_grads_list[_input].append(grads[j])
    ### END YOUR SOLUTION
