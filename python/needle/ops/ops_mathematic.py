...
class Transpose(TensorOp):
    ...
    def compute(self, a):
        ...
        self.axes = self.axes if self.axes else axes[-2:]
        if len(self.axes) == 2:
...
class ReLU(TensorOp):
    def compute(self, a):
        ### BEGIN YOUR SOLUTION
        x = (array_api.where(a > 0, 1, 0) * a).astype(array_api.float32)
        return x
        ### END YOUR SOLUTION

    def gradient(self, out_grad, node):
        ### BEGIN YOUR SOLUTION
        pos_idx = array_api.where(node.inputs[0].numpy() > 0, 1, 0)
        pos_idx = pos_idx.astype(array_api.float32)
        tens = Tensor(pos_idx, dtype="float32")
