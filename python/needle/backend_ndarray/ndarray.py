...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        ### BEGIN YOUR SOLUTION
        assert type(new_shape) in (list, tuple), "shape should in tuple or list type"
        if builtins.sum([i == -1 for i in new_shape]) == 1 and prod(new_shape) < 0:  # one -1
            new_shape = [self.size // abs(prod(new_shape)) if s == -1 else s for s in new_shape]
        assert prod(new_shape) == self.size, "must have equal length"
        strides, n = [], self.size
        for s in new_shape:
            strides.append(n // s)
            n //= s
        strides = [0] * len(new_shape) if not any(self._strides) else strides
        return NDArray.make(
    ...
    def permute(self, new_axes):
        ...
        ### BEGIN YOUR SOLUTION
        assert (new_axes is None or all(i in new_axes for i in range(self.ndim)))
        new_axes = new_axes if new_axes else [i for i in range(self.ndim)][::-1]
        new_shape = [self._shape[i] for i in new_axes]
        strides = [self._strides[i] for i in new_axes]
        return NDArray.make(
            tuple(new_shape), strides=tuple(strides), device=self.device, handle=self._handle
        )
        ### END YOUR SOLUTION
    ...
    def broadcast_to(self, new_shape):
        ...
        ### BEGIN YOUR SOLUTION
        assert len(self._shape) == len(new_shape), "must equal dim"
        for i, s in enumerate(new_shape):
            assert self._shape[i] == 1 or s == self._shape[i], "must same or self equal 1 "
        strides = []
        for i, s in enumerate(new_shape):
            if self._shape[i] == 1:
                strides.append(0)
    ...
    def __getitem__(self, idxs):
        ...
        ### BEGIN YOUR SOLUTION
        shape = tuple((sl.stop - 1 - sl.start + sl.step) // sl.step for sl in idxs)
        strides = tuple([self._strides[i] * sl.step for i, sl in enumerate(idxs)])
        offset = 0
        for i, sl in enumerate(idxs):
            offset += self._strides[i] * sl.start
        return self.make(shape, strides=strides, device=self._device, handle=self._handle, offset=offset)
        ### END YOUR SOLUTION
    ...
    def flip(self, axes):
        ...
        offset = builtins.sum([(self._shape[i]-1) * prod(self._shape[i+1:]) for i in axes])
        strides = tuple([-s if i in axes else s for i, s in enumerate(self._strides)])
        out = NDArray.make(self._shape, strides, device=self.device, handle=self._handle, offset=offset)
        out = out.compact()
        return out
        ### END YOUR SOLUTION
    ...
    def pad(self, axes):
        ...
        """
        ### BEGIN YOUR SOLUTION
        assert len(axes) == len(self._shape)
        assert type(axes[0]) is tuple
        shape = self._shape
        new_shape = [s + builtins.sum(axes[i]) for i, s in enumerate(shape)]
        new_array = full(new_shape, 0, dtype=self.dtype, device=self.device)
        slices = [slice(builtins.max(0, p[0]), s if not p[1] else -p[1]) for s, p in zip(shape, axes)]
...
def transpose(a, new_axes):
    return permute(a, new_axes)
